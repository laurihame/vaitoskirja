...
able to handle any types of time windows, see Section \ref{heuristic}. 
...
\end{align*}
\end{proof}
In general, the two theorems suggest that the expected CPU time is minimized 
when the initial degree $L_0$ is small. 
...
\citep{farahat, langville, ng, agosti}, in which the 
...
in which the goal is to construct a set of vehicle routes, serving a set of customers, satisfying 
the given time, capacity and precedence constraints.
...
\label{svsolution}
Let us present an exact constraint programming method for the single-vehicle DARP that 
...
REC($S,R_S$) shown in Algorithm \ref{alg01}, where $S$ denotes a sequence of nodes (initially $S=(0)$), $R_S$ denotes %based on a two-step feasibility check 
...
%\STATE \ \ Terminate recursion; \hfill (Feasible route.)
%\ENDIF
\STATE Rank the remaining nodes $i \in R_S$ and remove infeasible nodes from $R_S$; \hfill(See Section \ref{feasibility}.)
...
are removed from $R_S$ (see Section \ref{feasibility}).
...
\subsubsection{Adjacency matrix and hub vector}
\label{feasibility}
...
upper bounds $L_i$ of the time windows during run-time as described in \cite{hitsdarp}.
...
the one presented in \citep{desrosiers01}.
...
The difference in our formulation is that we define the feasibility of 
transitions as a function of the state of the vehicle (see Figure \ref{transitions01}).
...
In our formulation, a large hub score of node $i$ means that many 
...
\end{theorem}
\begin{proof}
 Suppose there exists a feasible sequence $(S,i_1,\ldots,i_h)$ for which $C((i_1,\ldots,i_h)) > \max_{i \in R_S} C(N(i))$.
Then, since $\{i_1,\ldots,i_h\} \subset N(i_1)$, we have $C(N(i_1)) \geq C((i_1,\ldots,i_h)) > \max_{i \in R_S} C(N(i))$, 
...
\subsubsection{A heuristic extension}
\label{heuristic}
Algorithm \ref{alg01} describes an exact procedure for maximizing the number of served customers
...
and stops when the set of remaining nodes in empty. By increasing $J$ the search space is expanded and
...
\bibliography{vk,eadarp,hitsdarp,hitsdarp2,dju,kuhmo}
